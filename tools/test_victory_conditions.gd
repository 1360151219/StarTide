...
func _initialize() -> void:
	...
	for level_id in ["level_02", "level_03", "level_04"]:
	...
	if not failed:
		print("VICTORY_OK survive=true elite=true boss=true combined=true timeout=true hints=true boundaries=true")
...
func _test_defeat_boss_mode(result_service: RefCounted) -> void:
	...
	_require(not level.victory.is_victory(149.99, level.duration, false, false), "Boss 存活时提前胜利")
	_require(level.victory.is_victory(100.0, level.duration, false, true), "击败 Boss 后没有立即胜利")
	_require(level.victory.is_timeout_failure(150.0, level.duration, false, false), "Boss 未击败时 150 秒没有失败")
	_require(not level.victory.is_timeout_failure(150.0, level.duration, false, true), "Boss 已击败仍判定超时")
