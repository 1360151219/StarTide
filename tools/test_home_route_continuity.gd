...
func _on_process_frame() -> void:
	...
	if frame_count == 3:
		...
		_require(screen.route_map.title_label.text == "风铃草原", "第一关信息没有进入远征路线区域")
		screen.route_map.move_by(1)
	elif frame_count == 8:
		...
		_require(screen.route_map.title_label.text == "金砂绿洲", "第二关信息没有更新")
		_require(screen.route_map.page_label.text == "2 / %d" % LevelCatalog.all().size(), "第二关页码没有更新")
		screen.route_map.move_by(1)
	elif frame_count == 13:
		...
		_require(screen.route_map.title_label.text == "彩晶火山", "第三关信息没有更新")
		_require(screen.route_map.page_label.text == "3 / %d" % LevelCatalog.all().size(), "第三关页码没有更新")
		if not failed:
			print("HOME_CONTINUITY_OK destinations=%d shell=stable data=updated" % LevelCatalog.all().size())
