...
const ENEMY_PREVIEW_TEXTURES := {
	"level_01": [preload("res://assets/art/enemies/green_grub.png"), preload("res://assets/art/enemies/starblight_slime.png")],
	"level_02": [preload("res://assets/art/enemies/duskwing_bat.png"), preload("res://assets/art/enemies/starblight_slime.png")],
	"level_03": [preload("res://assets/art/enemies/meteor_brute.png"), preload("res://assets/art/enemies/duskwing_bat.png")],
}
const PREVIEW_ART := {
...
func show_level(level: LevelConfig, progress_text: String, is_unlocked: bool) -> void:
	_ensure_built()
	current_level_id = level.level_id
	unlocked = is_unlocked
	var scene_atlas := AtlasTexture.new()
	scene_atlas.atlas = PREVIEW_ART[current_level_id]
	scene_atlas.region = Rect2(78, 62, 356, 356)
	scene_texture.texture = scene_atlas
	scene_texture.self_modulate = _floor_tint()
	preview_sprite.visible = unlocked
...
func _set_enemy_textures() -> void:
	var textures: Array = ENEMY_PREVIEW_TEXTURES[current_level_id]
	for index in range(enemy_nodes.size()):
		enemy_nodes[index].texture = textures[index % textures.size()]
		enemy_nodes[index].visible = unlocked


func _floor_tint() -> Color:
	return Color("e3f0dc") if current_level_id == "level_01" else Color("f2e4c8") if current_level_id == "level_02" else Color("e7c0b4")
