...
func _ready() -> void:
	...
	status_label = CharacterStyle.add_label(self, "远征中发现技能后，即可永久培养", 13, CharacterStyle.MUTED, Vector2(18, 494), Vector2(306, 50))
	status_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	status_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	reset_button = Button.new()
	reset_button.position = Vector2(336, 500)
...
func _build_skill_card(index: int) -> void:
	...
	skill_names.append(CharacterStyle.add_label(card, "", 18, CharacterStyle.INK, Vector2(96, 14), Vector2(214, 28)))
	skill_levels.append(CharacterStyle.add_label(card, "", 13, UiFactory.PRIMARY_DARK, Vector2(310, 14), Vector2(146, 28), HORIZONTAL_ALIGNMENT_RIGHT))
	var effect := CharacterStyle.add_label(card, "", 13, CharacterStyle.MUTED, Vector2(96, 46), Vector2(234, 58))
	effect.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	skill_effects.append(effect)
	var action := Button.new()
	action.position = Vector2(340, 54)
	action.size = Vector2(120, 50)
...
func _refresh(snapshot: Dictionary) -> void:
	...
	reset_button.text = "再次确认" if reset_armed else "↻  重置"
...
func _refresh_skill(index: int, skill: Dictionary) -> void:
	...
	if not discovered:
		skill_cards[index].tooltip_text = "在远征中获得该技能后开放培养"
		skill_cards[index].accessibility_name = "尚未发现的技能"
		button.disabled = true
		return
...
func _set_discovered_state(index: int, discovered: bool) -> void:
	...
	skill_names[index].visible = discovered
