...
func _update_star_lance(delta: float) -> void:
	...
	bolt_timer = data["cooldown"][skill_level]
...
func _update_sun_orbit(delta: float) -> void:
	...
	if orbit_hit_timer > 0.0:
		return
	orbit_hit_timer = data["hit_interval"][skill_level]
	var hit_anything := false
	for index in range(data["count"][skill_level]):
		var angle: float = orbit_phase + index * TAU / data["count"][skill_level]
		var orb_position: Vector2 = player.position + Vector2.from_angle(angle) * data["orbit_radius"][skill_level]
		for enemy in enemies.snapshot():
			if is_instance_valid(enemy) and enemy.position.distance_to(orb_position) <= enemy.radius + data["orb_radius"][skill_level]:
				hit_anything = true
				effects.add_effect(enemy.position, 34.0, Color("ffbf45"), 0.24, "sun_hit")
				enemies.damage_enemy(enemy, data["damage"][skill_level], Color("ffd765"))
	if hit_anything:
...
func _update_frost_tide(delta: float, elapsed: float) -> void:
	...
	pulse_timer = data["cooldown"][skill_level]
	pulse_visual_time = 0.3
	skill_released.emit("frost_tide")
	audio.play_sfx("skill_frost_tide", 0.0, rng.randf_range(0.97, 1.03))
	for enemy in enemies.snapshot():
		if is_instance_valid(enemy) and enemy.position.distance_to(player.position) <= data["radius"][skill_level] + enemy.radius:
			enemy.apply_slow(data["slow_factor"][skill_level], data["slow_duration"][skill_level], elapsed)
			enemies.damage_enemy(enemy, data["damage"][skill_level], Color("9ff4ff"))
