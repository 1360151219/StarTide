...
星芒枪的“贯星长枪”分支使用连续命中目标数表达贯穿成长：II 级最多连续命中 `2` 个目标，III 级最多 `3` 个目标，IV～V 级为无限贯穿。运行时以 `pierce = -1` 表示无限贯穿；每个投射物仍只能对同一个敌人命中一次，并继续受射程、存活时间和直线路径限制。

星芒枪基础数值按等级为：伤害 `[22, 25, 30, 32, 36]`、弹数 `[1, 2, 2, 3, 3]`、散布 `[0.13, 0.13, 0.13, 0.16, 0.19]`（索引对应 I～V 级）。分支的 `damage_multiplier` 乘在当前等级基础伤害上，`count`/`spread`/`pierce` 直接替换基础值。III 级两个分支的覆盖行分别沿用各自 II 级的定义，作用在 III 级自身的基础值上。以下为不含遗物与局外培养的裸数值：

“星雨齐射”（多弹·广域）：

| 等级 | 伤害 | 弹数 | 散布 |
|---|---:|---:|---:|
| II | 22.5（25 ×0.9） | 2 | 0.16 |
| III | 27.0（30 ×0.9） | 2 | 0.16 |
| IV | 25.6（32 ×0.8） | 3 | 0.19 |
| V | 26.64（36 ×0.74） | 5 | 0.26 |

“贯星长枪”（高伤·贯穿）：

| 等级 | 伤害 | 弹数 | 贯穿 |
|---|---:|---:|---|
| II | 27.5（25 ×1.1） | 1 | 连续命中 2 |
| III | 33.0（30 ×1.1） | 1 | 连续命中 3 |
| IV | 40.0（32 ×1.25） | 1 | 无限贯穿 |
| V | 49.68（36 ×1.38） | 1 | 无限贯穿 |

每个技能分支的 I→V 参考输出（`伤害 × 弹数 × 参考目标数 / 冷却`）受成长契约约束，当前允许区间为 `3.7 ～ 6.5` 倍；该上下限由 `tools/test_run_build.gd` 与 `tools/test_balance_contracts.gd` 守卫，调整分支数值时不得越界。所有技能的 III 级伤害必须严格等于 II 级的 `1.2` 倍，且 III 级不改动伤害以外的 runtime 字段（统一的“伤害提高 20%”节点）。

霜潮脉冲基础数值按等级为：伤害 `[20, 24, 28.8, 34, 41]`、冷却 `[2.45, 2.15, 2.15, 1.75, 1.35]`、半径 `[125, 155, 155, 195, 245]`、减速时长 `[1.6, 1.6, 1.6, 1.9, 2.3]`、减速系数 `[0.58, 0.58, 0.58, 0.45, 0.28]`（索引对应 I～V 级，减速系数越低减速越强）。分支的 `radius_multiplier`/`slow_duration_multiplier`/`cooldown_multiplier`/`damage_multiplier` 乘在当前等级基础值上；III 级两个分支的覆盖行分别沿用各自 II 级的定义。以下为不含遗物与局外培养的裸数值：

“永冻冰原”（广域·长控，仅放大范围与减速时长）：

| 等级 | 伤害 | 冷却 | 半径 | 减速时长 |
|---|---:|---:|---:|---:|
| II | 24 | 2.15 | 178.3（155 ×1.15） | 1.84（1.6 ×1.15） |
| III | 28.8 | 2.15 | 178.3（155 ×1.15） | 1.84（1.6 ×1.15） |
| IV | 34 | 1.75 | 247.7（195 ×1.27） | 2.41（1.9 ×1.27） |
| V | 41 | 1.35 | 330.8（245 ×1.35） | 3.17（2.3 ×1.38） |

“碎星寒潮”（高频·连控，降伤降半径换取更短冷却）：

| 等级 | 伤害 | 冷却 | 半径 |
|---|---:|---:|---:|
| II | 21.6（24 ×0.9） | 1.94（2.15 ×0.9） | 136.4（155 ×0.88） |
| III | 25.92（28.8 ×0.9） | 1.94（2.15 ×0.9） | 136.4（155 ×0.88） |
| IV | 29.24（34 ×0.86） | 1.51（1.75 ×0.86） | 165.75（195 ×0.85） |
| V | 33.62（41 ×0.82） | 1.107（1.35 ×0.82） | 200.9（245 ×0.82） |
...
- `elapsed` 是实际生存时间。战斗 HUD 和结算记录始终显示该值；精英或 Boss 存活时继续累计。
- `objective_elapsed` 是通关计时。第 1～4 关的精英实际出现后暂停，精英被击败后继续累计；暂停菜单与升级选择期间单局本身不推进，两者都不累计。

第 1～4 关统一要求 `objective_elapsed ≥ duration` 且精英已被击败，没有超时失败。第 5 关只以驺吾被击败作为胜利条件，同样没有超时失败；其 `duration = 150` 仅作为波次、Boss 登场进度和难度曲线的时间基准，不是战斗时限。
...
当前采用显式的前置加速梯度，让低等级更快形成基础构筑，同时保持到达 10 级所需的累计经验 `1332` 不变：

| 当前等级 → 下一等级 | 本级所需经验 | 到达下一等级的累计经验 |
|---|---:|---:|
| 1 → 2 | 24 | 24 |
| 2 → 3 | 48 | 72 |
| 3 → 4 | 76 | 148 |
...
怪物技能是否生效由每关 `enemy_ability_budget.disabled_ability_ids` 单独控制。当前第4、5关均禁用云角鹿的 `cloud_hart_sector` 与铃羽鸢的 `bellfeather_circle`；怪物仍按原波次生成，但不会进入对应技能的预警、执行或命中阶段。Boss 独立技能状态机不受该开关影响。

关闭这两项技能后，第4关普通波次的模型压力调整为接近第3关；第4关额外难度主要来自更长时长与 100 秒登场的云角领路者，不通过暗中增加刷怪量抵消技能关闭效果。
